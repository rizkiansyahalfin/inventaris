...
@php 
            $user = auth()->user();
    $unreadNotifications = ($user && is_object($user)) ? $user->notifications()->unread()->count() : 0;
    $recentNotifications = ($user && is_object($user)) ? $user->notifications()
        ->orderByRaw('read_at IS NULL DESC, created_at DESC')
        ->limit(10)
        ->get() : collect();
    $totalNotifications = ($user && is_object($user)) ? $user->notifications()->count() : 0;
@endphp
<header class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
    <div class="flex items-center justify-between h-16 px-4 sm:px-6">
        ...
        <div class="flex items-center space-x-4">
            ...
            <nav class="hidden sm:flex items-center space-x-2 text-sm">
                <a href="{{ route('dashboard') }}"
                    class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">Dashboard</a>
        ...
        <button onclick="toggleTheme()"
            ...
            title="Toggle Theme">
            <svg id="theme-icon-dark" class="hidden h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path>
            </svg>
            <svg id="theme-icon-light" class="hidden h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="hidden sm:block relative">
            <input type="text" placeholder="Cari..."
                class="w-64 pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        @auth
            <div class="relative" x-data="{ open: false }" x-init="
                ...
                <button @click="open = !open"
                    ...
                    <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15 17h5l-5 5v-5z M11 19H6.5A2.5 2.5 0 0 1 4 16.5v-9A2.5 2.5 0 0 1 6.5 5h11A2.5 2.5 0 0 1 20 7.5V13">
                        </path>
                    </svg>
                    @if($unreadNotifications > 0)
                        <span
                            class="absolute -top-1 -right-1 h-5 w-5 bg-red-500 rounded-full flex items-center justify-center text-xs text-white font-medium">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    class="notification-dropdown absolute right-0 mt-2 w-96 sm:w-[28rem] md:w-[32rem] lg:w-[36rem] bg-white dark:bg-gray-800 rounded-md shadow-lg py-1 z-50 border border-gray-200 dark:border-gray-700">
                    <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                        ...
                        @if($totalNotifications > 0)
                            <div class="flex items-center space-x-2">
                                <span class="text-xs text-gray-500 dark:text-gray-400">{{ $totalNotifications }}
                                    total</span>
                    ...
                    <div class="h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-gray-100 dark:scrollbar-track-gray-800"
                        style="overflow-y: scroll !important;" x-ref="scrollContainer">
                        ...
                        @else
                            ...
                            @foreach($groupedNotifications as $date => $notifications)
                                ...
                                @foreach($notifications as $notification)
                                    <div
                                        class="notification-item px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                                        <div class="flex items-start space-x-3">
                                            ...
                                            <div class="flex-1 min-w-0 max-w-full">
                                                <div class="flex flex-col space-y-1">
                                                    ...
                                                    <p
                                                        class="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 break-words leading-relaxed">
                    ...
                    <div class="px-4 py-2 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            <button @click="$refs.scrollContainer.scrollTop -= 100"
                                class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" title="Scroll Up">
                            ...
                            <button @click="$refs.scrollContainer.scrollTop += 100"
                                class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" title="Scroll Down">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if($unreadNotifications > 0)
                        <div class="px-4 py-2 border-t border-gray-200 dark:border-gray-700">
                            <form action="{{ route('notifications.mark-all-as-read') }}" method="POST" class="w-full">
                                ...
                                @method('PATCH')
                                <button type="submit"
                                    class="w-full text-xs text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-center">
                                    Tandai Semua Dibaca
                                </button>
                            </form>
                        </div>
                    ...
                    @if($totalNotifications > 10)
                        <div class="px-4 py-2 border-t border-gray-200 dark:border-gray-700">
                            <a href="{{ route('notifications.index') }}"
                                class="w-full text-xs text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-300 text-center block">
        ...
        @auth
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open"
                    ...
                    <span
                        class="hidden md:block text-sm font-medium text-gray-700 dark:text-gray-300">{{ $user->name }}</span>
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <a href="{{ route('profile.index') }}"
                        class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">Profil</a>
                    <a href="{{ route('system-configs.index') }}"
                        class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">Pengaturan</a>
                    <div class="border-t border-gray-200 dark:border-gray-700"></div>
                    <form method="POST" action="{{ route('logout') }}" onsubmit="return confirm('Yakin ingin keluar?')">
                        @csrf
                        <button type="submit"
                            class="w-full text-left block px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">Keluar</button>
                    </form>
                </div>
            </div>
        @else
