...
<div x-data="{ sidebarOpen: false }" class="relative">
    <!-- Overlay -->
    <div x-show="sidebarOpen" class="fixed inset-0 z-20 bg-black bg-opacity-40 md:hidden" @click="sidebarOpen = false" x-transition.opacity></div>
    <aside :class="{ '-translate-x-full': !sidebarOpen, 'translate-x-0': sidebarOpen }" class="fixed inset-y-0 left-0 z-30 w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col transition-transform duration-200 transform -translate-x-full md:translate-x-0 md:static md:inset-0">
        <!-- Logo & Close Button -->
        <div class="flex items-center justify-between h-16 px-4 border-b border-gray-200 dark:border-gray-800">
            ...
            <button class="md:hidden p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800" @click="sidebarOpen = false">
    ...
    <button class="fixed z-40 bottom-6 left-6 md:hidden bg-blue-600 text-white p-3 rounded-full shadow-lg focus:outline-none" @click="sidebarOpen = !sidebarOpen; $dispatch('close-nav')">
