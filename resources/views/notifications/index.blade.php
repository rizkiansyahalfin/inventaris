            ...
            <div class="p-6 md:p-8">
                ...
                @if($notifications->isEmpty())
                    <div class="text-center py-12">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Anda akan melihat notifikasi di sini ketika ada pembaruan.</p>
                    </div>
                @else
                    <div class="space-y-4">
                        @foreach($notifications as $notification)
                            <div id="notification-{{ $notification->id }}" class="notification-item p-4 border {{ $notification->isRead() ? 'border-gray-200 dark:border-gray-700' : 'border-blue-300 dark:border-blue-600' }} rounded-lg flex items-start space-x-4 transition-colors duration-300">
                                    ...
                                    <div class="mt-3 flex items-center space-x-4">
                                        @if(!$notification->isRead())
                                            <form action="{{ route('notifications.mark-as-read', $notification) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="text-xs font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">Tandai Dibaca</button>
                                            </form>
                                        @endif
                                        <form action="{{ route('notifications.destroy', $notification) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus notifikasi ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-xs font-medium text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-500">Hapus</button>
                                        </form>
                                        @if($notification->data && isset($notification->data['action_link']))
                                            <a href="{{ $notification->data['action_link'] }}" class="text-xs font-medium text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300">
